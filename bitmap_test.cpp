...
void test091()
{
   const unsigned int dim = 2995;
   bitmap_image image(dim,dim);

   for (unsigned int x=1010; x<dim; ++x) 
   {
     for (unsigned int y=1010; y<dim; ++y)
      {
        rgb_t col = jet_colormap[(x+y) % dim];
        image.set_pixel(x,y,col.red,col.green,col.blue);
     }
  ...
  image.save_image("test091_color_map._image.bmp");
}

void test092()
{
  const unsigned int dim = 8000;
  bitmap_image image(dim, dim);

  for (unsigned int x=7000; x<dim; ++x)
  {
    for (unsigned int y=7000; y<dim; ++y)
    {
       rgb_t col = jet_colormap[(x+y) % dim];
       image.set_pixel(x,y,col.blue,col.red,col.blue);
    }
  }

   image.save_image("test_092_color_map_image.bmp");
}

void test093()
{
  const unsigned int dim = 15000;
  bitmap_image image(dim, dim); 

  for (unsigned int x=14000; x<dim; ++x)
  {
    for (unsigned int y=14000; y<dim; ++y)
    {
      rgb_t col = jet_colormap[(x+y) % dim];
      image.set_pixel(x,y,col.green,col.green,col.red);
    }
  }

  image.save_image("test_093_color_map_image.bmp");
}
...
void test14()
{
   ...
   image.clear();
   checkered_pattern(128,256,220,bitmap_image::blue_plane,image);
   image.save_image("test14_checkered_03.bmp");

   image.clear();
   checkered_pattern(128,64,220,bitmap_image::green_plane,image);
   image.save_image("test14_checkered_04.bmp");
